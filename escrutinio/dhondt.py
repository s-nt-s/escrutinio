...
class dhondt():
    ...
    def __repr__(self):
        candidatures = sorted(self.dcandi.items(),
    ...
    def nseats(self, nseats):
        if type(nseats) is int and nseats > 0:
            ...
            raise AttributeError(
                'The number or seats value must be an integer greater than 0')
    ...
    def minper(self, minper):
        if type(minper) is float and minper > 0:
            ...
            raise AttributeError(
                'The minimum percentage value must be a float greater than 0')
    ...
    def dcandi(self, dcandi):
        if type(dcandi) is dict:
            ...
            try:
                ...
                raise AttributeError(
                    'The candidatures votes values must be integers')
        else:
    ...
    def bmp(self, bmp):
        if type(bmp) is bool:
            ...
            raise AttributeError(
                'The blank votes count for minimum percentage flag must be a Boolean ')
...
if __name__ == '__main__':
    ...
    baseparser = ArgumentParser(
        description="Performs the d'Hondt seats calculation")
    group_min = baseparser.add_argument_group('Minimum data')
    group_min.add_argument(
        'nseats', help='Number of seats for the calculation')
    group_min.add_argument(
        'minper', help='Minimun percentage of votes to enter in the calculation')
    group_min.add_argument(
        'datcan', help='Dictionary with the candidatures data')
    ...
    dcandi = dict((k, eval(v)) for (k, v) in [it.split(
        ':') for it in args['datcan'].replace("'", "").strip('{}').split(', ')])
    # Performs the dhont calc
    result = dhondt(nseats, minper, dcandi)
    # Returns data calc
    print(result)
    print('<seats: {0}>'.format(
        sorted(result.repre.items(), key=lambda p: p[1], reverse=True)))
